# Maestro by Equativ Printed CLI Agent Guide

This directory is a generated `equativ-maestro-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
equativ-maestro-pp-cli doctor --json
equativ-maestro-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
equativ-maestro-pp-cli which "<capability>" --json
equativ-maestro-pp-cli <command> --help
```
...
```bash
equativ-maestro-pp-cli <command> --agent
```
...
```bash
equativ-maestro-pp-cli <command> --help
equativ-maestro-pp-cli <command> --dry-run --agent
```
