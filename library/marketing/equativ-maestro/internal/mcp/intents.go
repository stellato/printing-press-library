...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("pre_review_reconciliation",
			mcplib.WithDescription("Surface orphaned deals/line items and budget-vs-spend mismatches across the synced book before a client review."),
			mcplib.WithBoolean("spend", mcplib.Description("Override the recipe's --spend value.")),
		),
		handlePreReviewReconciliation,
	)
}

// handlePreReviewReconciliation runs the pre_review_reconciliation recipe intent: Surface orphaned deals/line items and budget-vs-spend mismatches across the synced book before a client review.
func handlePreReviewReconciliation(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "spend", input["spend"], true)
