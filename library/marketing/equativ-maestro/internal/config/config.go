...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EQUATIV_MAESTRO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "equativ-maestro-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EQUATIV_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:EQUATIV_CLIENT_ID"
	}
	if v := os.Getenv("EQUATIV_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:EQUATIV_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("EQUATIV_MAESTRO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("EQUATIV_MAESTRO_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
