...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'equativ-maestro-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: equativ-maestro-pp-cli auth set-token <token>"+
			"\n      or: export EQUATIV_CLIENT_ID=<your-token>"+
			"\n      Get a key at: https://help.equativ.com/get-started-with-user-administration/create-and-manage-api-users"+
			"\n      Run 'equativ-maestro-pp-cli doctor' to check auth status.", err))
