...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Equativ Maestro",
...
Mint an OAuth2 bearer token via POST https://login.eqtv.io/oauth2/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to EQUATIV_CLIENT_ID (Client ID) and EQUATIV_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  equativ-maestro-pp-cli auth login

  # Explicit credentials
  equativ-maestro-pp-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" {
				clientID = strings.TrimSpace(os.Getenv("EQUATIV_CLIENT_ID"))
			}
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("EQUATIV_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or EQUATIV_CLIENT_ID/EQUATIV_CLIENT_SECRET)"))
			}
			...
			if tokenURL == "" {
				tokenURL = "https://login.eqtv.io/oauth2/token"
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $EQUATIV_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $EQUATIV_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("EQUATIV_MAESTRO_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return ""
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  equativ-maestro-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export EQUATIV_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export EQUATIV_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  equativ-maestro-pp-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  equativ-maestro-pp-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  equativ-maestro-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("EQUATIV_CLIENT_ID") != "" {
				envStillSet = "EQUATIV_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("EQUATIV_CLIENT_SECRET") != "" {
				envStillSet = "EQUATIV_CLIENT_SECRET"
			}
