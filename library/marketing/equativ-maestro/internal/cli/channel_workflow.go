...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  equativ-maestro-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  equativ-maestro-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"async-report", "businessunits", "cities", "countries", "deals", "deals-capping-timeframes", "deals-count-breakdown", "deals-kpi-trackers-definitions", "dmas", "fields-category", "inventory-types", "partners", "platforms", "regions", "report", "report-campaigns-delivery", "report-campaigns-pacing", "report-deals-delivery", "report-deals-pacing", "report-dimension-attributes", "report-dimensions", "report-field-restriction-rules", "report-line-items-delivery", "report-line-items-pacing", "report-troubleshooting", "states"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  equativ-maestro-pp-cli workflow status

  # Show status as JSON
  equativ-maestro-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("equativ-maestro-pp-cli")
			}
