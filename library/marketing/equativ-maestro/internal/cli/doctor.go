...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  equativ-maestro-pp-cli doctor
  equativ-maestro-pp-cli doctor --json
  equativ-maestro-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export EQUATIV_CLIENT_ID=<your-key>"
					report["auth_key_url"] = "https://help.equativ.com/get-started-with-user-administration/create-and-manage-api-users"
				} else {
					authConfigured = true
			...
			if os.Getenv("EQUATIV_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "EQUATIV_CLIENT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EQUATIV_CLIENT_ID")
			}
			if os.Getenv("EQUATIV_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "EQUATIV_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EQUATIV_CLIENT_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "equativ-maestro-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'equativ-maestro-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'equativ-maestro-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'equativ-maestro-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'equativ-maestro-pp-cli sync' to refresh."
	}
	return report
}
