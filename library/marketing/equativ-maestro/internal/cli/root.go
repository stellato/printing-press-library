...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "equativ-maestro-pp-cli",
		Short: `Equativ Maestro CLI — Every Maestro deal, avails forecast, and report — scriptable, with a local database and offline analytics the web UI an…`,
		Long: `Equativ Maestro CLI — Every Maestro deal, avails forecast, and report — scriptable, with a local database and offline analytics the web UI an…
...
Health check: run 'equativ-maestro-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("equativ-maestro-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'equativ-maestro-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAsyncReportCmd(flags))
	rootCmd.AddCommand(newAsyncReportGcpCmd(flags))
	rootCmd.AddCommand(newBusinessunitsCmd(flags))
	rootCmd.AddCommand(newCitiesCmd(flags))
	rootCmd.AddCommand(newCountriesCmd(flags))
	rootCmd.AddCommand(newDealsCmd(flags))
	rootCmd.AddCommand(newDmasCmd(flags))
	rootCmd.AddCommand(newInventoryTypesCmd(flags))
	rootCmd.AddCommand(newPartnersCmd(flags))
	rootCmd.AddCommand(newPlatformsCmd(flags))
	rootCmd.AddCommand(newRegionsCmd(flags))
	rootCmd.AddCommand(newReportCmd(flags))
	rootCmd.AddCommand(newSelfServeCmd(flags))
	...
	rootCmd.AddCommand(newCreativeBannerSizesPromotedCmd(flags))
	rootCmd.AddCommand(newDealPromotedCmd(flags))
	rootCmd.AddCommand(newFieldsCategoryPromotedCmd(flags))
	rootCmd.AddCommand(newFilterSuggestionsPromotedCmd(flags))
