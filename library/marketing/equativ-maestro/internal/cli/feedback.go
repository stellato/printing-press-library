...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "equativ-maestro-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EQUATIV_MAESTRO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EQUATIV_MAESTRO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "equativ-maestro-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/equativ-maestro-pp-cli/feedback.jsonl.
When ` + "`EQUATIV_MAESTRO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EQUATIV_MAESTRO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  equativ-maestro-pp-cli feedback list
  equativ-maestro-pp-cli feedback list --limit 5
  equativ-maestro-pp-cli feedback list --json`,
