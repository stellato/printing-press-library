...
//
// Known limitation: the key is the resource id alone, not network-qualified, so
// two networks that share a numeric id evict each other's cached row. This never
// serves wrong data — gamLoadResource filters reads by network prefix and
// live-fetches on a miss — but a user alternating --network values re-fetches
// instead of reading a stable mirror. Network-qualifying the key would also
// change the id surfaced by the `since` command, so it is left as-is.
