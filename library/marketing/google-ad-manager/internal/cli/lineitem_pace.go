...
fetch_failures:[{id,name,reason}]}.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// A nextPageToken (or a filled page) means the order has more line
			// items than this single page fetched. The ranking is then computed on
			// a partial set, so flag it rather than imply a complete worst-first
			// list — this command does not paginate.
			...
			if truncated {
				out["truncated"] = true
				out["truncation_note"] = fmt.Sprintf("only the first %d line items were fetched; the schedule-pace ranking may omit lower-ranked items. Re-run with a higher --limit for the full order.", limit)
				fmt.Fprintf(cmd.ErrOrStderr(), "warning: order has more line items than --limit=%d; ranking is computed on a partial set. Pass a higher --limit for the complete ranking.\n", limit)
			}
