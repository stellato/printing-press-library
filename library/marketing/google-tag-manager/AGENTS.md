# Google Tag Manager Printed CLI Agent Guide

This directory is a generated `google-tag-manager-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
google-tag-manager-pp-cli doctor --json
google-tag-manager-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
google-tag-manager-pp-cli which "<capability>" --json
google-tag-manager-pp-cli <command> --help
```
...
```bash
google-tag-manager-pp-cli <command> --agent
```
...
```bash
google-tag-manager-pp-cli <command> --help
google-tag-manager-pp-cli <command> --dry-run --agent
```
