...
Resource scoping:
  ...
  google-tag-manager-pp-cli sync

  # Sync specific resources only
  google-tag-manager-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  google-tag-manager-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  google-tag-manager-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  google-tag-manager-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  google-tag-manager-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/google-tag-manager-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "tagmanager":
		return db.UpsertTagmanager(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"tagmanager": "/tagmanager/v2/accounts",
	}
