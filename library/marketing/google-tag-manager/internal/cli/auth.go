...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Tag Manager",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  google-tag-manager-pp-cli auth setup\n  google-tag-manager-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://www.googleapis.com/auth/tagmanager.readonly")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export GTM_ACCESS_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  google-tag-manager-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://www.googleapis.com/auth/tagmanager.readonly"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  google-tag-manager-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export GTM_ACCESS_TOKEN=\"your-token-here\" # Google OAuth access token (tagmanager.readonly scope). From gcloud auth print-access-token or a service-account token.")
				fmt.Fprintf(w, "  google-tag-manager-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  google-tag-manager-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  google-tag-manager-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("GTM_ACCESS_TOKEN") != "" {
				envStillSet = "GTM_ACCESS_TOKEN"
			}
