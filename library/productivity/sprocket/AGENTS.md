# Sprocket Sports Printed CLI Agent Guide

This directory is a generated `sprocket-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sprocket-pp-cli doctor --json
sprocket-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sprocket-pp-cli which "<capability>" --json
...
```bash
sprocket-pp-cli <command> --help
sprocket-pp-cli <command> --dry-run --agent
```
