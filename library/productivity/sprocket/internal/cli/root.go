...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sprocket-pp-cli",
		Short: `Sprocket CLI — Your youth-soccer club's schedule, teams, and dues from the terminal — with merged multi-kid agendas, conflict detectio…`,
		Long: `Sprocket CLI — Your youth-soccer club's schedule, teams, and dues from the terminal — with merged multi-kid agendas, conflict detectio…
...
Health check: run 'sprocket-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sprocket-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sprocket-pp-cli profile list')")
	...
	rootCmd.AddCommand(newClubCmd(flags))
	rootCmd.AddCommand(newPaymentsCmd(flags))
	rootCmd.AddCommand(newProgramsCmd(flags))
	rootCmd.AddCommand(newRegistrationsCmd(flags))
	...
	rootCmd.AddCommand(newNovelAwayCmd(flags))
	rootCmd.AddCommand(newNovelConflictsCmd(flags))
	rootCmd.AddCommand(newNovelDeadlinesCmd(flags))
	rootCmd.AddCommand(newNovelIcalCmd(flags))
	rootCmd.AddCommand(newNovelNextCmd(flags))
	rootCmd.AddCommand(newNovelOwedCmd(flags))
