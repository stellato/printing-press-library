...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sprocket-pp-cli doctor
  sprocket-pp-cli doctor --json
  sprocket-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SPROCKET_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://jfcsoccer.sprocketsports.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("SPROCKET_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SPROCKET_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SPROCKET_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "sprocket-pp-cli"
						verifyPath := "/api/club-users/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'sprocket-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'sprocket-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'sprocket-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'sprocket-pp-cli sync' to refresh."
	}
	return report
}
