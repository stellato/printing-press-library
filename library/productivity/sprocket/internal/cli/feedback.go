...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sprocket-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SPROCKET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SPROCKET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sprocket-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sprocket-pp-cli/feedback.jsonl.
When ` + "`SPROCKET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SPROCKET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sprocket-pp-cli feedback list
  sprocket-pp-cli feedback list --limit 5
  sprocket-pp-cli feedback list --json`,
