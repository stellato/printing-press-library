...
native streaming instead of polling.`,
  ...
  sprocket-pp-cli tail --interval 10s

  # Tail a specific resource
  sprocket-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  sprocket-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Reject unknown resources up front: without this, an unknown name
			// polls "/<name>" forever (or once under dogfood) instead of failing
			// fast, so a typo hangs the terminal.
			...
			if !valid {
				_ = cmd.Usage()
				return usageErr(fmt.Errorf("unknown resource %q; valid resources: %v", resource, known))
			}
