...
type Config struct {
	...
	SprocketToken string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://jfcsoccer.sprocketsports.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SPROCKET_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sprocket-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SPROCKET_TOKEN"); v != "" {
		cfg.SprocketToken = v
		cfg.markEnvOverride("SprocketToken")
		cfg.AuthSource = "env:SPROCKET_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.SprocketToken != "" {
		cfg.AuthSource = "config"
	}
	...
	// Club subdomain shorthand: SPROCKET_CLUB=<sub> targets
	// https://<sub>.sprocketsports.com. Validated as a bare hostname label so a
	// stray value cannot redirect the host. SPROCKET_BASE_URL (full URL) wins
	// when both are set.
	if v := os.Getenv("SPROCKET_CLUB"); v != "" {
		clean := true
		for _, r := range v {
		...
		if clean {
			cfg.BaseURL = "https://" + v + ".sprocketsports.com"
		}
	}
	// Base URL override (full URL; also used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SPROCKET_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SprocketToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SPROCKET_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":          c.SprocketToken,
			"SPROCKET_TOKEN": c.SprocketToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SprocketToken = ""
	delete(c.envOverrides, "SprocketToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SprocketToken"] {
			out.SprocketToken = c.fileConfig.SprocketToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SprocketToken":
		c.fileConfig.SprocketToken = c.SprocketToken
	}
}
