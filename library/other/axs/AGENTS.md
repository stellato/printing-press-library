# SRAM AXS Printed CLI Agent Guide

This directory is a generated `axs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
axs-pp-cli doctor --json
axs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
axs-pp-cli which "<capability>" --json
axs-pp-cli <command> --help
```
...
```bash
axs-pp-cli <command> --help
axs-pp-cli <command> --dry-run --agent
```
