...
Resource scoping:
  ...
  axs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  axs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  axs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  axs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  axs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/axs-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "bikes":
		return db.UpsertBikes(data)
	case "components":
		return db.UpsertComponents(data)
	case "devicetypes":
		return db.UpsertDevicetypes(data)
	case "linkedids":
		return db.UpsertLinkedids(data)
	case "models":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"account":                      "/users/self/public_accessgroups/",
		"activities":                   "https://api.quarqnet.com/api/v2/activities/",
		...
		"summaries":                    "https://api.quarqnet.com/api/v2/activitysummaries/",
		"summaries-componentsummaries": "https://api.quarqnet.com/api/v2/componentsummaries/",
		"units":                        "/advancedunits/",
	}
