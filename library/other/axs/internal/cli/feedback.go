...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "axs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AXS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AXS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "axs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/axs-pp-cli/feedback.jsonl.
When ` + "`AXS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AXS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  axs-pp-cli feedback list
  axs-pp-cli feedback list --limit 5
  axs-pp-cli feedback list --json`,
