...
native streaming instead of polling.`,
  ...
  axs-pp-cli tail --interval 10s

  # Tail a specific resource
  axs-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  axs-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !isTailKnownResource(resource) {
				return fmt.Errorf("unknown resource %q (known: %v)", resource, tailKnownResources())
			}
...
func isTailKnownResource(resource string) bool {
	for _, known := range tailKnownResources() {
		if resource == known {
			return true
		}
	}
