...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  axs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  axs-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "activities", "bikes", "components", "linkedids", "models", "notifications", "summaries", "summaries-componentsummaries", "units"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  axs-pp-cli workflow status

  # Show status as JSON
  axs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("axs-pp-cli")
			}
