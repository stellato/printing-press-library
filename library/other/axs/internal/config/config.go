...
type Config struct {
	...
	SramAxsToken  string            `toml:"axs_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://nexus.quarqnet.com/api/v2",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AXS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "axs-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SRAM_AXS_TOKEN"); v != "" {
		cfg.SramAxsToken = v
		cfg.markEnvOverride("SramAxsToken")
		cfg.AuthSource = "env:SRAM_AXS_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.SramAxsToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.SramAxsToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SRAM_AXS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"axs_token":      c.SramAxsToken,
			"SRAM_AXS_TOKEN": c.SramAxsToken,
			"token":          c.SramAxsToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SramAxsToken = ""
	delete(c.envOverrides, "SramAxsToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SramAxsToken"] {
			out.SramAxsToken = c.fileConfig.SramAxsToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SramAxsToken":
		c.fileConfig.SramAxsToken = c.SramAxsToken
	}
}
