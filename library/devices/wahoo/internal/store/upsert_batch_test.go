...
func TestSearchPlansQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("plans", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchPlans(query, 10)
		if err != nil {
			t.Fatalf("SearchPlans(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchPlans(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesPowerZonesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed power_zones table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPowerZonesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("power-zones", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "power-zones").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "power_zones")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count power_zones: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("power_zones count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestSearchRoutesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("routes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchRoutes(query, 10)
		if err != nil {
			t.Fatalf("SearchRoutes(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchRoutes(%q) returned no results", query)
		}
	}
}
...
// TestUpsertBatch_PopulatesWorkoutFileUploadsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed workout_file_uploads table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesWorkoutFileUploadsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("workout-file-uploads", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "workout-file-uploads").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "workout_file_uploads")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count workout_file_uploads: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("workout_file_uploads count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_PopulatesWorkoutsPlansTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed workouts_plans table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesWorkoutsPlansTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "workouts_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "workouts_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "workouts_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("workouts_plans", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "workouts_plans").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "workouts_plans")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count workouts_plans: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("workouts_plans count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandWorkoutsPlansGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandWorkoutsPlansGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "workouts_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("workouts_plans", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("workouts_plans count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "workouts_id")
	}
}

// TestUpsertBatch_KeysWorkoutsPlansByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysWorkoutsPlansByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "workouts_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "workouts_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "workouts_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "workouts_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("workouts_plans", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("workouts_plans count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "workouts_id" = ?`, "workouts_plans")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by workouts_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("workouts_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesWorkoutSummaryTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed workout_summary table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesWorkoutSummaryTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("workout_summary", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "workout_summary").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "workout_summary")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count workout_summary: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("workout_summary count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandWorkoutSummaryGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandWorkoutSummaryGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("workout_summary", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("workout_summary count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "workouts_id")
	}
}

// TestUpsertBatch_KeysWorkoutSummaryByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysWorkoutSummaryByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("workout_summary count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "workouts_id" = ?`, "workout_summary")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by workouts_id: %v", err)
	}
	if parentMatches != 2 {
