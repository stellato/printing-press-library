...
In local mode: searches locally synced data only.`,
  ...
  wahoo-pp-cli search "error timeout"

  # Force local search only
  wahoo-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  wahoo-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  wahoo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'wahoo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "plans":
				results, err = db.SearchPlans(query, limit)
			case "routes":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchPlans(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search plans failed: %w", searchErr)
					}
