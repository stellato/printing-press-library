...
Resource scoping:
  ...
  wahoo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  wahoo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wahoo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wahoo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wahoo-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "power-zones":
		return db.UpsertPowerZones(data)
	case "routes":
		return db.UpsertRoutes(data)
	case "user":
		return db.UpsertUser(data)
	case "workout-file-uploads":
		return db.UpsertWorkoutFileUploads(data)
	case "workouts":
		return db.UpsertWorkouts(data)
	case "workouts_plans":
		return db.UpsertWorkoutsPlans(data)
	case "workout_summary":
		return db.UpsertWorkoutSummary(data)
	default:
