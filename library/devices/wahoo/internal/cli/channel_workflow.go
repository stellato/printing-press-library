...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  wahoo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  wahoo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"plans", "power-zones", "routes", "workouts"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  wahoo-pp-cli workflow status

  # Show status as JSON
  wahoo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("wahoo-pp-cli")
			}
