...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wahoo-pp-cli doctor
  wahoo-pp-cli doctor --json
  wahoo-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "wahoo-pp-cli login"
					report["auth_docs_url"] = "https://developers.wahooligan.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("WAHOO_CLOUD_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "WAHOO_CLOUD_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WAHOO_CLOUD_OAUTH2")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("wahoo-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'wahoo-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'wahoo-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'wahoo-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'wahoo-pp-cli sync' to refresh."
	}
	return report
}
