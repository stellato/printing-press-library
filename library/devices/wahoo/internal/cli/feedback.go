...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "wahoo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WAHOO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WAHOO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wahoo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/wahoo-pp-cli/feedback.jsonl.
When ` + "`WAHOO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WAHOO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wahoo-pp-cli feedback list
  wahoo-pp-cli feedback list --limit 5
  wahoo-pp-cli feedback list --json`,
