...
type Config struct {
	...
	WahooCloudOauth2 string          `toml:"cloud_oauth2"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("WAHOO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "wahoo-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("WAHOO_CLOUD_OAUTH2"); v != "" {
		cfg.WahooCloudOauth2 = v
		cfg.markEnvOverride("WahooCloudOauth2")
		cfg.AuthSource = "env:WAHOO_CLOUD_OAUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.WahooCloudOauth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("WAHOO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("WAHOO_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("WAHOO_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.WahooCloudOauth2 != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:WAHOO_CLOUD_OAUTH2"
		}
		return "Bearer " + c.WahooCloudOauth2
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.WahooCloudOauth2 = ""
	delete(c.envOverrides, "WahooCloudOauth2")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["WahooCloudOauth2"] {
			out.WahooCloudOauth2 = c.fileConfig.WahooCloudOauth2
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "WahooCloudOauth2":
		c.fileConfig.WahooCloudOauth2 = c.WahooCloudOauth2
	}
}
