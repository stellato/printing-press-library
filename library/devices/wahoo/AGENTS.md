# Wahoo Cloud API Printed CLI Agent Guide

This directory is a generated `wahoo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
wahoo-pp-cli doctor --json
wahoo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
wahoo-pp-cli which "<capability>" --json
wahoo-pp-cli <command> --help
```
...
```bash
wahoo-pp-cli <command> --help
wahoo-pp-cli <command> --dry-run --agent
```
