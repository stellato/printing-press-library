# Ikon Pass Printed CLI Agent Guide

This directory is a generated `ikon-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ikon-pp-cli doctor --json
ikon-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ikon-pp-cli which "<capability>" --json
ikon-pp-cli <command> --help
```
...
```bash
ikon-pp-cli <command> --help
ikon-pp-cli <command> --dry-run --agent
```
