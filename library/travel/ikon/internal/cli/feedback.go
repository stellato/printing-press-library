...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ikon-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IKON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IKON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ikon-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ikon-pp-cli/feedback.jsonl.
When ` + "`IKON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IKON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ikon-pp-cli feedback list
  ikon-pp-cli feedback list --limit 5
  ikon-pp-cli feedback list --json`,
