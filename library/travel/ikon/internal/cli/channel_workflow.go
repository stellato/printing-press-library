...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ikon-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ikon-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"passes", "reservations", "resorts"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ikon-pp-cli workflow status

  # Show status as JSON
  ikon-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ikon-pp-cli")
			}
