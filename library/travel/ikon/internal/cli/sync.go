...
Resource scoping:
  ...
  ikon-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ikon-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ikon-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ikon-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ikon-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ikon-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "benefits":
		return db.UpsertBenefits(data)
	case "resorts":
		return db.UpsertResorts(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"passes":       "/api/v2/my-products",
		"reservations": "/api/v2/reservations",
		"resorts":      "/api/v2/resorts",
	}
