...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Ikon",
...
func requiredAuthCookies() []string {
	raw := []string{"session", "soft_session", "csrf_token"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for ikonpass.com.
...
profile by name when the installed backend supports it.`,
		Example: `  ikon-pp-cli auth login --chrome
  ikon-pp-cli auth login --browser
  ikon-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  ikon-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  ikon-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := "ikonpass.com"
			accountDomain := "account.ikonpass.com"
			...
			// are already on disk.
			var cookies string
			var accountCookies string
			fromPressAuth := false
			if pressAuthPath, err := exec.LookPath("press-auth"); err == nil {
				paCookies, paErr := tryPressAuth(pressAuthPath, domain)
				paAccountCookies, paAccountErr := tryPressAuth(pressAuthPath, accountDomain)
				if paErr == nil && paAccountErr == nil && paCookies != "" && paAccountCookies != "" {
					fmt.Fprintf(w, "Loaded cookies via press-auth for %s.\n", domain)
					cookies = paCookies
					accountCookies = paAccountCookies
					fromPressAuth = true
				} else if paErr != nil || paAccountErr != nil {
					if paErr == nil {
						paErr = paAccountErr
					}
			...
			if !fromPressAuth {
				...
				if err != nil {
					fmt.Fprintln(w, "No cookie extraction tool found; trying live browser session fallback.")
					liveCookies, liveErr := extractLiveCookies(domain)
					liveAccountCookies, liveAccountErr := extractLiveCookies(accountDomain)
					if liveErr == nil && liveAccountErr == nil && liveCookies != "" && liveAccountCookies != "" {
						fmt.Fprintf(w, "Loaded cookies from live browser session for %s.\n", domain)
						cookies = liveCookies
						accountCookies = liveAccountCookies
					} else {
						if liveErr == nil {
							liveErr = liveAccountErr
						}
						fmt.Fprintf(w, "Live browser fallback failed: %v\n", liveErr)
						fmt.Fprintln(w, "")
						fmt.Fprintln(w, red("No cookie extraction tool found."))
						fmt.Fprintln(w, "")
						if runtime.GOOS == "windows" {
							fmt.Fprintln(w, "pycookiecheat does not support Windows. Start Chrome with remote debugging or install a cross-platform reader:")
							fmt.Fprintln(w, "  chrome.exe --remote-debugging-port=9222")
							...
							fmt.Fprintln(w, "  cargo install cookie-scoop-cli     # Rust")
							fmt.Fprintln(w, "Or start Chrome with --remote-debugging-port=9222 for live browser fallback.")
						}
						return authErr(fmt.Errorf("no cookie tool found"))
					}
				}

				if cookies == "" || accountCookies == "" {
					...
					accountCookies, err = extractCookies(tool, accountDomain, profileDir)
					if err != nil {
						return authErr(fmt.Errorf("extracting account cookies: %w", err))
					}
					...
					if accountCookies == "" {
						return authErr(fmt.Errorf("cookie tool returned no cookies for %s", accountDomain))
					}
				}
			...
			accountCookieMap := parseCookieString(accountCookies)
			for k, v := range accountCookieMap {
				cookieMap[k] = v
			}
			requiredCookies := requiredAuthCookies()
			...
			// client's cookie jar.
			topLevelJarCookies := map[string]string{}
			if v, ok := cookieMap["session"]; ok {
				topLevelJarCookies["session"] = v
			}
			if err := client.WriteCookieJarFromMap(domain, topLevelJarCookies); err != nil {
				fmt.Fprintf(w, "warning: persisting cookie jar: %v (cookies still saved to config)\n", err)
			}
			accountJarCookies := map[string]string{}
			for _, name := range []string{"soft_session", "csrf_token", "clientside-cookie", "login-via-auth0", "locale", "branding", "_itw_iaa_prod_session", "amp-prod-session_cart"} {
				if v, ok := cookieMap[name]; ok {
					accountJarCookies[name] = v
				}
			}
			if err := client.WriteCookieJarFromMap(accountDomain, accountJarCookies); err != nil {
				fmt.Fprintf(w, "warning: persisting account cookie jar: %v (cookies still saved to config)\n", err)
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ikon-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  ikon-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: ikonpass.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ikon-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  ikon-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func extractViaPycookiecheat(tool cookieTool, domain, profileDir string) (string, error) {
	...
	if cookiePath != "" {
		script = fmt.Sprintf(
			...
			pythonStringLiteral("https://"+cleanDomain), pythonStringLiteral(filepath.ToSlash(cookiePath)),
		)
	} else {
		script = fmt.Sprintf(
			`import json; from pycookiecheat import chrome_cookies; print(json.dumps(chrome_cookies(%s)))`,
			pythonStringLiteral("https://"+cleanDomain),
		)
	}
...
func extractViaCDP(targetURL, port string) (string, error) {
	...
	for _, tab := range tabs {
		if strings.Contains(tab.URL, domain) {
			if err := validateCDPWebSocketURL(tab.WebSocketDebuggerURL, port); err != nil {
				return "", err
			}
...
func validateCDPWebSocketURL(rawURL, expectedPort string) error {
	u, err := url.Parse(rawURL)
	if err != nil {
		return fmt.Errorf("parsing CDP WebSocket URL: %w", err)
	}
	if u.Scheme != "ws" && u.Scheme != "wss" {
		return fmt.Errorf("rejecting CDP WebSocket URL with unexpected scheme %q", u.Scheme)
	}
	if u.Port() != expectedPort {
		return fmt.Errorf("rejecting CDP WebSocket URL on unexpected port %q", u.Port())
	}
	host := u.Hostname()
	...
	return fmt.Errorf("rejecting CDP WebSocket URL on non-loopback host %q", host)
}
