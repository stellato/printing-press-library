...
func (r *recordingRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	r.calls++
	r.lastAuthorization = req.Header.Get("Authorization")
...
func TestClientSendsSavedBearerToken(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	rec := &recordingRoundTripper{}
	token := "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImFvNi1PYzAyQlZHTF9GbnBDeE5JMiJ9." +
		strings.Repeat("a", 430) + "." + strings.Repeat("b", 344)
	cfg := &config.Config{BaseURL: "http://example.test", AccessToken: token}
	c := New(cfg, time.Second, 0)
	c.HTTPClient = &http.Client{Transport: rec}
	c.NoCache = true
	...
	if got, want := rec.lastAuthorization, "Bearer "+token; got != want {
		t.Fatalf("Authorization header = %q, want %q", got, want)
	}
}

func TestClientDoesNotSendCookieStringAsBearerToken(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	rec := &recordingRoundTripper{}
	cfg := &config.Config{BaseURL: "http://example.test", AccessToken: "session=abc; csrf_token=xyz"}
	c := New(cfg, time.Second, 0)
	c.HTTPClient = &http.Client{Transport: rec}
	c.NoCache = true
	...
	if got := rec.lastAuthorization; got != "" {
