...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://account.ikonpass.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("IKON_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ikon-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("IKON_BASE_URL"); v != "" {
		cfg.BaseURL = v
		cfg.markEnvOverride("BaseURL")
	}
	return cfg, nil
}
...
func (c *Config) configForSave() Config {
	...
	out.Headers = cloneStringMap(out.Headers)
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BaseURL":
		c.fileConfig.BaseURL = c.BaseURL
	case "Headers":
		c.fileConfig.Headers = cloneStringMap(c.Headers)
	}
}
...
func (c *Config) save() error {
	...
	if err := atomicWriteFile(c.Path, data, 0o600); err != nil {
		return err
	}
