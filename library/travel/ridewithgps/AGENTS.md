# Ride with GPS Printed CLI Agent Guide

This directory is a generated `ridewithgps-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ridewithgps-pp-cli doctor --json
ridewithgps-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ridewithgps-pp-cli which "<capability>" --json
ridewithgps-pp-cli <command> --help
```
...
```bash
ridewithgps-pp-cli <command> --agent
```
...
```bash
ridewithgps-pp-cli <command> --help
ridewithgps-pp-cli <command> --dry-run --agent
```
