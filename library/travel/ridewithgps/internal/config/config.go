...
type Config struct {
	...
	RidewithgpsApiKey    string            `toml:"api_key"`
	RidewithgpsAuthToken string            `toml:"auth_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("RIDEWITHGPS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ridewithgps-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("RIDEWITHGPS_API_KEY"); v != "" {
		cfg.RidewithgpsApiKey = v
		cfg.markEnvOverride("RidewithgpsApiKey")
		cfg.AuthSource = "env:RIDEWITHGPS_API_KEY"
	}
	...
	if v := os.Getenv("RIDEWITHGPS_AUTH_TOKEN"); v != "" {
		cfg.RidewithgpsAuthToken = v
		cfg.markEnvOverride("RidewithgpsAuthToken")
	}
	...
	if cfg.AuthSource == "" && cfg.RidewithgpsApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("RIDEWITHGPS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.RidewithgpsApiKey
	if token == "" {
		return ""
	}
	if c.RidewithgpsApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.RidewithgpsApiKey = token
	delete(c.envOverrides, "RidewithgpsApiKey")
	c.updateFileConfigField("RidewithgpsApiKey")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	c.RidewithgpsApiKey = ""
	delete(c.envOverrides, "RidewithgpsApiKey")
	c.RidewithgpsAuthToken = ""
	delete(c.envOverrides, "RidewithgpsAuthToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RidewithgpsApiKey"] {
			out.RidewithgpsApiKey = c.fileConfig.RidewithgpsApiKey
		}
		if c.envOverrides["RidewithgpsAuthToken"] {
			out.RidewithgpsAuthToken = c.fileConfig.RidewithgpsAuthToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RidewithgpsApiKey":
		c.fileConfig.RidewithgpsApiKey = c.RidewithgpsApiKey
	case "RidewithgpsAuthToken":
		c.fileConfig.RidewithgpsAuthToken = c.RidewithgpsAuthToken
	}
}
