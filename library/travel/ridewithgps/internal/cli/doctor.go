...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ridewithgps-pp-cli doctor
  ridewithgps-pp-cli doctor --json
  ridewithgps-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export RIDEWITHGPS_API_KEY=<your-key>"
					report["auth_docs_url"] = "https://ridewithgps.com/api"
				} else {
					authConfigured = true
			...
			if os.Getenv("RIDEWITHGPS_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "RIDEWITHGPS_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RIDEWITHGPS_API_KEY")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.RidewithgpsAuthToken
				}
				recordAdditionalAuthEnv("RIDEWITHGPS_AUTH_TOKEN", configuredValue)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ridewithgps-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ridewithgps-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ridewithgps-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'ridewithgps-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ridewithgps-pp-cli sync' to refresh."
	}
	return report
}
