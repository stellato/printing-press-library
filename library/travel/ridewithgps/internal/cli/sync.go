...
Resource scoping:
  ...
  ridewithgps-pp-cli sync

  # Sync specific resources only
  ridewithgps-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ridewithgps-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ridewithgps-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ridewithgps-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ridewithgps-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "points_of_interest":
		return db.UpsertPointsOfInterest(data)
	case "routes":
		return db.UpsertRoutes(data)
	case "sync-json":
		return db.UpsertSyncJson(data)
	case "trips":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"members":            "/api/v1/members.json",
		"points_of_interest": "/api/v1/points_of_interest.json",
		"routes":             "/api/v1/routes.json",
		"sync-json":          "/api/v1/sync.json",
		"trips":              "/api/v1/trips.json",
	}
