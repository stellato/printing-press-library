...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ridewithgps-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RIDEWITHGPS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RIDEWITHGPS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ridewithgps-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ridewithgps-pp-cli/feedback.jsonl.
When ` + "`RIDEWITHGPS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RIDEWITHGPS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ridewithgps-pp-cli feedback list
  ridewithgps-pp-cli feedback list --limit 5
  ridewithgps-pp-cli feedback list --json`,
