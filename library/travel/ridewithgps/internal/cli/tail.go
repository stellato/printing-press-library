...
func newTailCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"mcp:read-only": "true", "pp:happy-args": "--resource=routes"},
...
native streaming instead of polling.`,
  ...
  ridewithgps-pp-cli tail --interval 10s

  # Tail a specific resource
  ridewithgps-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  ridewithgps-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if resource == "" {
				return fmt.Errorf("resource name required (e.g., 'tail routes')")
			}
			// Reject unknown resources up front so an invalid argument exits
			// non-zero rather than polling a 404 endpoint forever.
			knownResource := false
			...
			if !knownResource {
				_ = cmd.Usage()
				return usageErr(fmt.Errorf("unknown resource %q; run 'ridewithgps-pp-cli tail --json' to list known resources", resource))
			}
			...
			// Resolve the real API path for the resource (e.g. routes ->
			// /api/v1/routes.json). The naive "/" + resource form hits the web
			// HTML page, not the JSON API, and yields invalid JSON.
			path, err := syncResourcePath(resource)
			if err != nil {
				_ = cmd.Usage()
				return usageErr(fmt.Errorf("cannot tail %q: %w", resource, err))
			}
