...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ridewithgps-pp-cli",
		Short: `Ridewithgps CLI — Your whole Ride with GPS library, offline — bulk export, gear mileage, and ride analytics no other tool ships.`,
		Long: `Ridewithgps CLI — Your whole Ride with GPS library, offline — bulk export, gear mileage, and ride analytics no other tool ships.

Highlights (not in the official API docs):
  • export   Export many routes or trips to GPX, TCX, CSV, or KML files in one command — with cue sheets included, ready to side-load onto a Garmin, Wahoo, or Hammerhead. The bulk export Ride with GPS itself won'…
...
Health check: run 'ridewithgps-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ridewithgps-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ridewithgps-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPointsOfInterestCmd(flags))
	...
	rootCmd.AddCommand(newNovelDedupCmd(flags))
	rootCmd.AddCommand(newNovelEventRoutesCmd(flags))
	rootCmd.AddCommand(newNovelExportCmd(flags))
	rootCmd.AddCommand(newNovelGearCmd(flags))
	rootCmd.AddCommand(newNovelRecordsCmd(flags))
	rootCmd.AddCommand(newNovelStatsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAuthTokensPromotedCmd(flags))
	rootCmd.AddCommand(newSyncJsonPromotedCmd(flags))
