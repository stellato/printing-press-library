...
In local mode: searches locally synced data only.`,
  ...
  ridewithgps-pp-cli search "error timeout"

  # Force local search only
  ridewithgps-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  ridewithgps-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  ridewithgps-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ridewithgps-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "points_of_interest":
				results, err = db.SearchPointsOfInterest(query, limit)
			case "routes":
				results, err = db.SearchRoutes(query, limit)
			case "trips":
				results, err = db.SearchTrips(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchPointsOfInterest(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search points_of_interest failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRoutes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search routes failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTrips(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search trips failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ridewithgps-pp-cli/data.db)")
