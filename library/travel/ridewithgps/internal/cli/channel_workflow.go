...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ridewithgps-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ridewithgps-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "events", "members", "points_of_interest", "routes", "sync-json", "trips"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ridewithgps-pp-cli workflow status

  # Show status as JSON
  ridewithgps-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ridewithgps-pp-cli")
			}
